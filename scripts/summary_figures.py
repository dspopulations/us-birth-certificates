...
def load_variant(variant: str) -> dict:
    ...
    age_red = np.array(
        [
            1.0 - (p[age == a] * n[age == a]).sum() / (theta_cell[age == a] * n[age == a]).sum()
            for a in range(len(AGE_LEVELS))
        ]
    )
    # Per ethnicity recorded / true rate per 10k.
    rec10k = np.array([r[race == j].sum() / n[race == j].sum() * 1e4 for j in range(len(RACE_LEVELS))])
    true10k = np.array(
        [(p[race == j] * n[race == j]).sum() / n[race == j].sum() * 1e4 for j in range(len(RACE_LEVELS))]
    )
    return {
        "variant": variant,
        "fit_dir": fit_dir,
...
def fig_funnel(c: dict, b: dict) -> None:
    ...
    ax.set_title("Ascertainment funnel: most true DS births never reach the certificate")
    ...
    data = pd.DataFrame(
        {
            "stage": ["natural", "born_C", "born_B", "recorded_corrected", "recorded_raw"],
...
def fig_age_reduction(c: dict, b: dict) -> None:
    ...
    ax.plot(x, c["age_reduction"], "-o", color=styles.COLOUR_BLUE, label="Variant C (pin recording)")
    ax.plot(x, b["age_reduction"], "--s", color=styles.COLOUR_ORANGE, label="Variant B (free recording)")
    ax.set_xticks(x)
...
def fig_ethnicity(c: dict, b: dict) -> None:
    ...
    ax.bar(x, hi - lo, bottom=lo, width=0.5, color=styles.COLOUR_ORANGE, alpha=0.55, label="True (model, C..B)")
    ax.plot(x, c["rec10k"], "o", color=styles.COLOUR_RED, label="Recorded (certificate)")
    ax.set_xticks(x)
...
def fig_education(c: dict) -> None:
    ...
    ax.errorbar(
        x, c["ete_mean"], yerr=err, fmt="o", color=styles.COLOUR_BLUE, capsize=3, label="Posterior (data)"
    )
    ...
    ax.set_title("Higher education -> much higher termination (data overrides the prior)")
    ax.legend(fontsize=8)
    data = pd.DataFrame(
        {
