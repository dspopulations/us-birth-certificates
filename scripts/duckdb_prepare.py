...
def _retype_columns(con: duckdb.DuckDBPyConnection) -> None:
    """Narrow every raw NVSS column to its DuckDB storage type."""
...
def _add_derived_columns(con: duckdb.DuckDBPyConnection) -> None:
    """Add the empty derived columns that the steps below populate."""
...
def _add_id_column(con: duckdb.DuckDBPyConnection) -> None:
...
def _set_year(con: duckdb.DuckDBPyConnection) -> None:
    print("Setting year...")
...
def _set_ca_down_c(con: duckdb.DuckDBPyConnection) -> None:
...
def _set_down_ind(con: duckdb.DuckDBPyConnection) -> None:
...
def _set_mage_c(con: duckdb.DuckDBPyConnection) -> None:
...
def _set_p_ds_lb_nt(con: duckdb.DuckDBPyConnection) -> None:
...
def _load_reference_csv(
    con: duckdb.DuckDBPyConnection, csv_path: str, table_name: str
) -> None:
    """Read a reference CSV and materialise it as a same-shape DuckDB table.

    Used for the prevalence / reduction-rate / case-weight lookup tables
    that get UPDATE-joined into ``us_births`` below. DuckDB's replacement
    scan resolves ``df`` from this function's own local scope.
    """
    print(f"Reading {csv_path}")
    df = pd.read_csv(csv_path).convert_dtypes()  # noqa: F841 -- read by the SQL below
    print(f"Creating table {table_name}")
    con.execute(f"CREATE TABLE {table_name} AS SELECT * FROM df")


def _set_p_ds_lb_wt(con: duckdb.DuckDBPyConnection) -> None:
...
def _set_mrace_c(con: duckdb.DuckDBPyConnection) -> None:
...
def _set_mhisp_c(con: duckdb.DuckDBPyConnection) -> None:
...
def _set_mracehisp_c(con: duckdb.DuckDBPyConnection) -> None:
...
def _set_p_ds_lb_wt_mage(con: duckdb.DuckDBPyConnection) -> None:
    _load_reference_csv(
        con,
        "./data/us-births-estimated-prevalence-maternal-age-1989-2018.csv",
        "us_births_est_prevalence_age",
    )
...
def _set_p_ds_lb_nt_reduc(con: duckdb.DuckDBPyConnection) -> None:
    _load_reference_csv(
        con, "./data/us-births-reduction-rates-1989-2024.csv", "reduction_rate_year"
    )
...
def _set_ds_case_weight(con: duckdb.DuckDBPyConnection) -> None:
    _load_reference_csv(
        con, "./data/us-births-ds-rec-weights.csv", "ds_case_weights"
    )
...
def _load_ethnicity_prevalence_reference(con: duckdb.DuckDBPyConnection) -> None:
    # NOTE: unlike the other reference tables above, this one is not yet
    # UPDATE-joined back into us_births — p_ds_lb_wt_ethn / p_ds_lb_nt_ethn
    # stay NULL. Loaded here (unchanged from the pre-refactor behaviour) so
    # the table is available in the database for a follow-up derivation.
    _load_reference_csv(
        con,
        "./data/us-births-estimated-prevalence-ethnicity-2000-2018.csv",
        "us_births_est_prevalence_ethnicity",
    )


def _compact_database(out_db_temp: pathlib.Path, out_db: pathlib.Path) -> None:
    """Copy the working DB into the final compacted DB, then reset the temp copy."""
...
def combine_all() -> None:
    ...
    try:
        ...
        _set_mrace_c(con)
        _set_mhisp_c(con)
        _set_mracehisp_c(con)
        _set_p_ds_lb_wt_mage(con)
        _set_p_ds_lb_nt_reduc(con)
        _set_ds_case_weight(con)
        _load_ethnicity_prevalence_reference(con)
    finally:
        print("Closing connection...")
        con.close()

    _compact_database(out_db_temp, out_db)
