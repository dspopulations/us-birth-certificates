...
- `environment.yml` installs it from the public git tag `v0.7.1` in its pip layer: `dse-research-utils[notebook,dependence,tuning,io] @ git+https://github.com/dseinternational/research.git@v0.7.1#subdirectory=src/python`. A commented local-dev override installs it editable from the sibling checkout instead — `-e ../../dseinternational/research/src/python[...]` (note the `../../` — this repo lives under `dspopulations/`, not `dseinternational/`), which must be cloned alongside this one.
