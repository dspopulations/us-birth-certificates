...
> **Update (2026-06-28):** all three outstanding items were implemented and the DB rebuilt + verified. Multi-race is now `mrace_c = 5` / `mracehisp_c = 6` (1,108,304 records, concentrated 2014+; the selection model routes code 6 to the existing "Unknown" race cell, so model cells are unchanged). `p_ds_lb_wt_mage_reduc` was renamed to `p_ds_lb_nt_reduc`. The inline prevalence table was externalised to `us-births-surveillance-prevalence-1989-2024.csv` (byte-identical values). The output-neutral tidy-ups (Morris-formula centralisation in `chance.py`, `MAGER9`/`DOWNS` clamp tightening) were applied earlier. Still genuinely optional: a prep-time assertion that the MBRACE two-digit branch is never reached, and promoting multi-race to its own selection-model group (rather than the Unknown cell).
