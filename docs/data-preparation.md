...
Example range specs: uint16 `DATAYEAR`/`BIRYR`/`DOB_YY`=(1989, None), `DOB_TT`/`DBWT`=(0, 9999), `DWGT_R`=(100, 999), `PWGT_R`=(75, 999); uint8 `DOB_MM`=(1, 12), `MAGER`=(12, 50), `DOWNS`=(0, 255), `UCA_DOWNS`=(1, 9), `NO_CONGEN`=(0, 1), `DPLURAL`=(1, 5), `APGAR5`=(0, 99). Columns with `(None, None)` (e.g. `DMAGE`, `MRACE`, `MAR`, `DMEDUC`) still get robust parse + non-integer nulling + the dtype cast but no range clamping. `string_cols` covers `SEX`, marital flags (`MAR_P`, `DMAR`), the `AB_*`, `CA_*` (including `CA_DOWN`/`CA_DOWNS`), `RF_*`, `LD_*` flags, `BFED`, `WIC`; `float16_cols = [vars.BMI]`.
...
- **59 fully-empty records.** Across all 142.9 M rows, 59 carry no `year` and no source fields at all (no `dob_yy`/`datayear`, no maternal age, no birth weight; only 2 have a birth month). They are empty import artefacts (~0.00004%) that land NULL on every year-keyed derived column - they are exactly the 59 residual `mage_c` nulls noted above. They can be dropped with `DELETE FROM us_births WHERE year IS NULL`.
- **`umeduc` is an empty column.** The NCHS 2003 transition-file record layout names the 1989-cert education item `UMEDUC`, but that column is unpopulated in the build - the 1989-cert education is carried by `DMEDUC` (present 1989-2008 in our extract, including 2003) and `MEDUC` carries the 2003-cert education.
- **Cross-era field renames and availability** are now recorded per-variable in `src/dspopulations_us_birth_certificates/variables.py`. Analytically important cases: maternal marital status is the `MAR` column only for 2003-2013 (it is `DMAR` in 1989-2002 and again from 2014); infant `SEX` (M/F) exists only from 2003 (1989-2002 is the un-imported `CSEX`, coded 1/2); `DPLURAL` carries a 5th category ("Quintuplet or higher") through 2019 and collapses to four from 2020; `DMETH_REC` is the detailed 7-code delivery recode in 2003-2004 but the collapsed 3-value recode from 2006 (and is absent in 2005, when the detailed role moves to `RDMETH_REC`); birth weight `DBWT`, five-minute Apgar `APGAR5`/`APGAR5R`, and gestation `GESTREC10` are renamed from the 1989-cert `DBIRWT`/`FMAPS`/`FMAPSR`/`GESTAT10` (and `GESTAT10` uses different gestational-week boundaries).
